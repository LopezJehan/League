...
buildTable <- function(number, data){
  ...
  colonne2_new <- c()
  for(elt in colonne2){
    colonne2_new <- c(colonne2_new, paste(elt, paste0('<img src="perso/', elt, '.png" height="30"></img>')))
  }
  ...
  colonne5_new <- c()
  for(elt in colonne5){
    colonne5_new <- c(colonne5_new, paste(paste0('<img src="perso/', elt, '.png" height="30"></img>'), elt))
  }
  ...
  tab <- data.frame(colonne1, colonne2_new, colonne3, colonne4, colonne5_new, colonne6)
  colnames(tab) <- c("Blue Side", teams$team[1], teamskda[1], teamskda[2], teams$team[2], "Red Side")
